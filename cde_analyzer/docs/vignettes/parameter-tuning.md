...
## 3. Phase 2: Phrase Mining — The Critical Tuning Area
...
| Parameter | Small (~1K) | Large (~22K) | Default | How to set |
|-----------|------------|-------------|---------|-----------|
| `min_parent_tinyids` | **2** | 20 | 20 | `--set "min_parent_tinyids=N"` |
| `k_max` | 25 | 25 | 25 | `--set "k_max=N"` |
| `k_min` | 3 | 3 | 3 | `--set "k_min=N"` |
| `min_field_count` | **3-4** | 6 | 6 | `--set "min_field_count=N"` |
| `min_tokens` | 3 | 3 | 3 | `--set "min_tokens=N"` |
...
**With allcde01 (22,743 CDEs)**, the default of 20 works well because the
broad corpus has many high-frequency phrases.
...
For large datasets, the default of 6 filters noise effectively.

### `k_max` and `k_min`

These control the k-mer mining window (maximum and minimum phrase length
in tokens). The defaults of 25 and 3 work well across dataset sizes.

Increase `k_max` only if you suspect very long repeated phrases (>25 words).
The `--dedup` pre-pass automatically catches whole-text duplicates exceeding
`k_max`, so extremely long phrases are handled regardless.
...
## 5. Setting Parameters — Three Methods
...
| Parameter | Phase | Default | Small (~1K) | Large (~22K) | Flag |
|-----------|-------|---------|------------|-------------|------|
| `workers` | All | 0 | 0 | 4-8 | `--set "workers=N"` |
| `min_parent_tinyids` | 2 | 20 | **2** | 20 | `--set "min_parent_tinyids=N"` |
| `k_max` | 2 | 25 | 25 | 25 | `--set "k_max=N"` |
| `k_min` | 2 | 3 | 3 | 3 | `--set "k_min=N"` |
| `min_field_count` | 2 | 6 | **3-4** | 6 | `--set "min_field_count=N"` |
| `min_tokens` | 2 | 3 | 3 | 3 | `--set "min_tokens=N"` |
| `min_prefix_tinyids` | 1, 2 | 2 | 2 | 2 | `--set "min_prefix_tinyids=N"` |
...
Also check `k_max` — values above 30 significantly increase mining time
with diminishing returns. The `--dedup` pre-pass (enabled by default) catches
the very long duplicates that high `k_max` would find.
