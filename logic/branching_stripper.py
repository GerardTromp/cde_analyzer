...
all 7 variants per CDE simultaneously.
...
Variant dependency graph::
              ...
              │
    original ─┼─ MTSTPF (inst_full then inst_sub)
              │
              ├─ inst_full ──── MTSFPT (+ temporal + phrase)
              │
              ├─ inst_sub ───── MFSTPT (+ temporal + phrase)
              │
              ├─ inst_full+sub ─ MTSTPT (+ temporal + phrase)
              │
              └─ temporal ───── MFSFPT (+ phrase)

With field-aware splits (v0.9.8+), inst_full and inst_sub operate on
different text spans: inst_full matches the group prefix (e.g. "PROMIS"),
inst_sub matches the separator+suffix (e.g. " - Anxiety").  This makes
MT+ST genuinely different from MT alone — all 7 variants are distinct.
...
def _strip_single_cde_nway(
    ...
    # ── Compute inst_full+sub intermediate (for MTSTPF, MTSTPT) ──

    need_inst_both = any(
        "inst_full" in VARIANT_STAGES[v] and "inst_sub" in VARIANT_STAGES[v]
        for v in variants
    )
    inst_both_data = None
    if need_inst_both and inst_full_data is not None and "inst_sub" in stages:
    ...
    if "MTSTPF" in variants and inst_both_data is not None:
        results["MTSTPF"] = copy.deepcopy(inst_both_data)
